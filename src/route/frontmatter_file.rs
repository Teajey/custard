...
fn post_inner(
    ...
    let response = custard_lib::single::query(keeper, name, query, sort_key, order_desc, intersect)
...
fn get_inner(
    ...
    let response = custard_lib::single::get(keeper, name, sort_key, order_desc)
        .ok_or(StatusCode::NOT_FOUND)?;
